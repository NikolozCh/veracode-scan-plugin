...
public class VeracodeNotifier extends Notifier {
     ...
     * global.jelly. This class extends the {@link hudson.tasks.BuildStepDescriptor
    ...
    public static final class VeracodeDescriptor extends BuildStepDescriptor<Publisher> {
        ...
        public FormValidation doTestConnection(@QueryParameter("gvid") final String gv_id,
                ...
                @QueryParameter("phost") final String p_host,
                @QueryParameter("pport") final String p_port,
                @QueryParameter("puser") final String p_user,
                @QueryParameter("ppassword") final String p_password) {
            return FormValidationUtil.checkConnection(gv_id, gv_key,
                    _proxy ? new ProxyBlock(p_host, p_port, p_user, p_password) : null);
        }

        public FormValidation doCheckGvid(@QueryParameter("gvid") String gv_id,
                @QueryParameter("gvkey") String gv_key) throws IOException, ServletException {
            return FormValidationUtil.checkMutuallyInclusiveFields(gv_id, gv_key, vidDisplayName,
                    vkeyDisplayName, vkeyIHelpTextName);
        }

        public FormValidation doCheckGvkey(@QueryParameter("gvid") String gv_id,
                @QueryParameter("gvkey") String gv_key) throws IOException, ServletException {
            return FormValidationUtil.checkMutuallyInclusiveFields(gv_key, gv_id, vkeyDisplayName,
                    vidDisplayName, vidIHelpTextName);
        }

        public FormValidation doCheckVid(@QueryParameter("vid") String v_id,
                @QueryParameter("vkey") String v_key) throws IOException, ServletException {
            boolean hasJobCredentials = !StringUtil.isNullOrEmpty(v_id)
                    && !StringUtil.isNullOrEmpty(v_key);
            ...
            if (hasGlobalCredentials) {
                if (hasJobCredentials) {
                    ...
                    return FormValidationUtil.checkFields(v_id, v_key, vidDisplayName,
                            vkeyDisplayName, vkeyIHelpTextName);
                } else {
                    return FormValidationUtil.checkMutuallyInclusiveFields(v_id, v_key,
                            vidDisplayName, vkeyDisplayName, vkeyIHelpTextName);
                }
            } else {
                return FormValidationUtil.checkFields(v_id, v_key, vidDisplayName, vkeyDisplayName,
                        vkeyIHelpTextName);
            }
        }

        public FormValidation doCheckVkey(@QueryParameter("vid") String v_id,
                @QueryParameter("vkey") String v_key) throws IOException, ServletException {
            boolean hasJobCredentials = !StringUtil.isNullOrEmpty(v_id)
                    && !StringUtil.isNullOrEmpty(v_key);
            boolean hasGlobalCredentials = hasGlobalCredentials();
            ...
            if (hasGlobalCredentials) {
                if (hasJobCredentials) {
                    ...
                    return FormValidationUtil.checkFields(v_key, v_id, vkeyDisplayName,
                            vidDisplayName, vidIHelpTextName);
                } else {
                    return FormValidationUtil.checkMutuallyInclusiveFields(v_key, v_id,
                            vkeyDisplayName, vidDisplayName, vidIHelpTextName);
                }
            } else {
                return FormValidationUtil.checkFields(v_key, v_id, vkeyDisplayName, vidDisplayName,
                        vidIHelpTextName);
            }
        }
        ...
        public FormValidation doCheckFilenamepattern(
                @QueryParameter("filenamepattern") String filename_pattern,
                @QueryParameter("replacementpattern") String replacement_pattern)
                throws IOException, ServletException {
            return FormValidationUtil.checkMutuallyInclusiveFields(filename_pattern,
                    replacement_pattern, filenamePatternDisplayName, replacementPatternDisplayName,
                    null);
        }

        public FormValidation doCheckReplacementpattern(
                @QueryParameter("filenamepattern") String filename_pattern,
                @QueryParameter("replacementpattern") String replacement_pattern)
                throws IOException, ServletException {
            return FormValidationUtil.checkMutuallyInclusiveFields(replacement_pattern,
                    filename_pattern, replacementPatternDisplayName, filenamePatternDisplayName,
                    null);
        }
            ...
            if (!StringUtil.isNullOrEmpty(timeout)) {
                try {
                    ...
                    return FormValidation
                            .error(String.format("%s is not a valid number.", timeout));
                }
            }
            return FormValidation.ok();
        }
        ...
        public ListBoxModel doFillCriticalityItems(
                @QueryParameter("criticality") String criticality) {
            ListBoxModel items = new ListBoxModel();
            for (String s : criticalityDisplayNames)
                items.add(new ListBoxModel.Option(s, s.replace(" ", ""),
                        s.replace(" ", "").equals(criticality)));
            return items;
        }
        ...
        /**
         * Initializes this class' instance fields using the data in the specified
         * {@link net.sf.json.JSONObject JSONObject} object.
         * <p>
         *
         * This method is intended to be called after the data in the {@code formData}
         * object has been encrypted.
         *
         * @param formData
         */
         ...
         * sensitive encrypted data to the submitted form associated with the specified
         * {@link org.kohsuke.stapler.StaplerRequest StaplerRequest} object.
         * <p>
         * This is done mostly as a safety measure and is probably not necessary if the
         * form's data was already encrypted.
         *
         * @param req
        ...
        /**
         * Checks to see if the id and key fields are empty despite having valid
         * information in the global settings page
         *
         * @param id  String
         * @param key String
         * @return boolean
    ...
    public String getVid() {
        return EncryptionUtil
                .decrypt((this._credentials != null) ? this._credentials.getVid() : null);
    }

    public String getVkey() {
        return EncryptionUtil
                .decrypt((this._credentials != null) ? this._credentials.getVkey() : null);
    }
    ...
    public boolean prebuild(AbstractBuild<?, ?> build, BuildListener listener) {
        ...
        // only copy if remote workspace and the checkbox to copy from remote to master
        // is unchecked
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
        ...
        try {
            if (isRemoteWorkspace) {
                ...
                if (copyRemoteFiles) {
                    localWorkspaceDir = new File(build.getParent().getRootDir(),
    ...
    private void getScanResults(AbstractBuild<?, ?> build, BuildListener listener)
        ...
        if (descriptor.getProxy()) {
            proxy = new ProxyBlock(descriptor.getPhost(), descriptor.getPport(),
        ...
        try {
            ...
            ScanHistory scanHistory = XmlUtil.newScanHistory(buildInfoXML, detailedReportXML,
                    build);
            Credentials credentials = Credentials.create(id, key);
            String xmlApiHost = credentials.getRegion().getXmlApiHost();
            build.addAction(new VeracodeAction(scanHistory, xmlApiHost));
        } catch (Exception e) {
            build.addAction(new VeracodeAction());
            throw e;
        }
